...
def show_progress(target, x, label, best_loss, save_path=None):
    verts, colors = vec_to_parts(x)
    rendered      = render_triangles(verts, colors)
...
def show_final(target, x, solver_name, best_loss):
    verts, colors = vec_to_parts(x)
    rendered      = render_triangles(verts, colors)

    fig, axes = plt.subplots(1, 2, figsize=(8, 4))
    axes[0].imshow(target,   vmin=0, vmax=1); axes[0].set_title("Target");            axes[0].axis("off")
    axes[1].imshow(rendered, vmin=0, vmax=1); axes[1].set_title(f"{solver_name} Result"); axes[1].axis("off")
...
def run_ga(target):
    ...
    ga              = GeneticAlgorithm(target=target)
    VISUALISE_EVERY = 20

    while not ga.is_done():
        stats = ga.step()
        gen   = stats["generation"]
        print(f"Gen {gen:4d} | best={stats['best_loss']:.6f} "
              f"| avg={stats['avg_loss']:.6f} "
              f"| stagnation={ga.stagnation_count}/{STAGNATION_GENS}")

        if gen % VISUALISE_EVERY == 0 or ga.is_done():
            show_progress(target, stats["best_x"],
                          label=f"GA_gen{gen:04d}",
                          best_loss=stats["best_loss"])

    final = ga._stats()
    return final["best_x"], final["best_loss"]
...
def main():
    ...
    if args.solver == "ga":
        best_x, best_loss = run_ga(target)
        solver_label = "GA"
    else:
